...
class EncoderCore:
    ...
    def openInputDataset(self, in_fn):
        ...
        self.in_band_2 = self.in_ds.GetRasterBand(2)
        self.in_band_2.SetNoDataValue(self.noData)
    ...
    def createOutputDataset(self):
        ...
        for i in range(0, self.in_ds.RasterYSize, self.in_yBlockSize):  
            ...
            for j in range(0, self.in_ds.RasterXSize, self.in_xBlockSize):  
                ...
                data_1 = self.in_band_1.ReadAsArray(j, i, cols, rows)
                data_2 = self.in_band_2.ReadAsArray(j, i, cols, rows)
                
                if  self.in_band_1.DataType == 3: # GDT_Int16
                    ...
                    self.out_ds.GetRasterBand(4).WriteArray(np.bitwise_and(np.right_shift(data_2, 8), 0xff), j, i)
...
def extant_folder(x):
    """
    'Type' for argparse - checks if output folder exists, yes: return true (means to use existing output!!! UPDATE MODE) no: create folder + return false.
    """
